    ...
    {%- for variant in e.variants() %}
        if (type == "{{ variant.name()|var_name|unquote }}") {
            ...
            {%- when Type::Optional(_) %}
                ...
                let _{{field.name()|var_name|unquote}} = data["{{field.name()|var_name|unquote}}"] as? {{field.type_()|rn_type_name(ci)}}
                {% else -%}
            ...
            {%- else %}
            {% if field.type_()|inline_optional_field(ci) -%}
            guard let _{{field.name()|var_name|unquote}} = data["{{field.name()|var_name|unquote}}"] as? {{field.type_()|rn_type_name(ci)}} else { throw SdkError.Generic(message: "Missing mandatory field {{field.name()|var_name|unquote}} for type {{ type_name }}") }
            {%- else -%}
